...
import dataloader
def parse_args():
    desc = desc = "Tensorflow implementation of ResNet"
...
def building_block(x_init, channels, is_training=True, use_bias=True, scope = 'buildingblock'):
    '''
    this is pre_activation version, BN+ReLU before conv
    :param x_init: input
    :param channels: output channels(equals filters)
    :param is_training: whether this block is trainable or not
    :param use_bias: whether use bias or not
    :param scope: the scope name
    :return: building block sub graph
    '''
    with tf.variable_scope(scope):
        # pre_activation
        x = tf.layers.batch_normalization(x=x_init, epsilon=1e-05, training=is_training)
        x = tf.nn.relu(features=x)

        # conv
        x = tf.layers.conv2d(inputs=x, filters=channels, kernel_size=3, strides=(1,1), use_bias=use_bias, padding='same')
        # BN + ReLU
        x = tf.layers.batch_normalization(x=x, epsilon=1e-05, training=is_training)
        x = tf.nn.relu(features=x)
        # conv
        x = tf.layers.conv2d(inputs=x, filters=channels, kernel_size=3, strides=(1, 1), use_bias=use_bias,
                             padding='same')
        x = x + x_init
        return x
...
class ResNet():
    def __init__(self, sess, args):
        self.sess = sess
        self.model_name = 'ResNet' + str(args.res_n)
    ...
    def network(self, x, is_training=True, reuse=False):
        '''
        :param x:
        :param is_training: whether training or inference
        :param reuse: whether to reuse scope
        :return: network graph
        '''
        with tf.variable_scope(name_or_scope='network', reuse=reuse):
            if self.res_n < 50:
                res_block = building_block
            else:
                res_block = bottleneck_block
            residual_list = get_residual_layer(self.res_n)
            ch = 64
            x = tf.layers.conv2d(inputs=x, filters=ch, kernel_size=3, strides=(1, 1), padding='same')
            # --------first block--------
            for i in range(residual_list[0]):
                x = res_block(x, channels=ch, is_training=is_training, downsample=False, scope='resblock0_' + str(i))
            # --------second block--------
            x = res_block(x, channels=ch * 2, is_training=is_training, downsample=True, scope='resblock1_0')
            for i in range(1, residual_list[1]):
                x = res_block(x, channels=ch * 2, is_training=is_training, downsample=False, scope='resblock1_' + str(i))
            # --------third block--------
            x = res_block(x, channels=ch * 4, is_training=is_training, downsample=True, scope='resblock2_0')
            for i in range(1, residual_list[2]):
                x = res_block(x, channels=ch * 4, is_training=is_training, downsample=False, scope='resblock2_' + str(i))
            # --------fourth block--------
            x = res_block(x, channels=ch * 8, is_training=is_training, downsample=True, scope='resblock_3_0')
            for i in range(1, residual_list[3]):
                x = res_block(x, channels=ch * 8, is_training=is_training, downsample=False, scope='resblock_3_' + str(i))

            x = x = tf.layers.batch_normalization(x=x, epsilon=1e-05, training=is_training)
            x = tf.nn.relu(features=x)
            x = tf.layers.average_pooling2d(inputs=x, pool_size=2, strides=2, padding='SAME')
            x = tf.layers.flatten(x)
            x = tf.layers.Dense(x, units=self.label_dim)
