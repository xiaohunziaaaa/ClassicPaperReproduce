...
import numpy as np
from nets import cg_conv, cd_conv
...
# nets
Genrator = cg_conv()
Discriminator = cd_conv()
G_vars = Genrator.vars()
D_vars = Discriminator.vars()
G_sample = Genrator(Z, Y)
D_real = Discriminator(X, Y)
D_fake = Discriminator(G_sample, Y, reuse=True)
G_vars = Genrator.vars()
D_vars = Discriminator.vars()
print(G_vars)
print(D_vars)
...
for epoch in range(epochs):
    for step in range(total_steps):
        ...
        # train Discriminator
        D_train_, D_loss_ = sess.run([D_optimizer, D_loss], feed_dict={X:x_batch, Y:y_batch, Z:z_batch})
        # train Generator
        z_batch = sample_Z(batch_size, 100)
        G_train_, G_loss_= sess.run([G_optimizer, G_loss], feed_dict={Z:z_batch, Y:y_batch})
        if global_steps % 500 == 0:
            ...
            y_sample[:,3] = 1
            samples = sess.run(G_sample, feed_dict={Z: sample_Z(16, 100), Y:y_sample})
            fig = plot(samples)
            plt.savefig('cGANout/{}.png'.format(str(i).zfill(3)), bbox_inches='tight')
            i += 1
            plt.close(fig)
        global_steps += 1